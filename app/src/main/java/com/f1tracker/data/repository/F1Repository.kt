...
class F1RepositoryImpl @Inject constructor(
    ...
    override suspend fun getInstagramFeed(forceRefresh: Boolean): Result<List<com.f1tracker.data.models.InstagramPost>> = withContext(Dispatchers.IO) {
        try {
            ...
            val baseUrl = "https://boxcreate.github.io/boxboxbox/data/f1_feed.json"
    ...
    override suspend fun getInstagramReels(forceRefresh: Boolean): Result<List<com.f1tracker.data.models.InstagramPost>> = withContext(Dispatchers.IO) {
        try {
            ...
            // Fetch fresh - append timestamp ONLY if forcing refresh to bypass all caches
            val baseUrl = "https://boxcreate.github.io/boxboxbox/data/f1_reels.json"
            val url = if (forceRefresh) "$baseUrl?t=${System.currentTimeMillis()}" else baseUrl
    ...
    override suspend fun getHighlights(): Result<List<com.f1tracker.data.models.HighlightVideo>> = withContext(Dispatchers.IO) {
        try {
            val url = "https://boxcreate.github.io/boxboxbox/data/f1_highlights.json"
    ...
    override suspend fun getYouTubeVideosFromJson(): Result<List<com.f1tracker.data.models.YouTubeVideo>> = withContext(Dispatchers.IO) {
        try {
            val url = "https://boxcreate.github.io/boxboxbox/data/f1_youtube.json"
